...
Never recall a key number, but do not re-measure one either: **the measured table is in the
kit's `docs/hardware-facts.md` section 7** (Edge Grinder's thirty-five bindable keys plus
SHIFT and CTRL, Layer 9h). Check it first and use this skill only for a key that is not in it,
or when a number in source has no measurement behind it. Both ports read the keyboard straight
from the System VIA, so the internal matrix number is what the code carries.
