...
The rule in both ports: **do not write hardware code from recalled facts.** Set the register,
read the effect back, record the readback verbatim, then build on it. Every fact in the kit's
`docs/hardware-facts.md` came from this procedure, and a fact that cannot say which probe
produced it is a recollection.
...
2. **Boot a machine of the project's target model** (`CLAUDE.md` "Target"; `create_machine
   model: "Master"` or `"B-DFS1.2"`). The MOS and the memory map differ, and a fact measured on
   the wrong model is a fact about a different machine. `run_until_prompt` to the BASIC prompt.
...
6. **Record the readback verbatim**, with the date and the emulator, in the layer doc
   (`docs/layer-N-*.md`), and as a row in `docs/decisions.md` if the fact changes a decision.
   Add the one-line form to the rules file's "Confirmed hardware facts (measured, not assumed)"
   list. "Measured" without the numbers cannot be re-checked.
...
8. **Then, because emulators disagree, `beeb-cross-emulator`** for anything that switches the
   display bank, changes CRTC shape mid-frame or displays memory the MOS would not.
