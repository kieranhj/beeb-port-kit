...
1. **Suspect uninitialised state first** when the symptom is reported that way. List the
   candidates from the source: every `SKIP` block, every region reclaimed from the MOS
   (`&0400-&07FF` language workspace, `&0800-&0BFF` sound and soft-key workspace, the top of
   the stack page), every flag that is only ever *cleared* by an event. The project's
   `docs/memory-map.md` and `CLAUDE.md` "Memory" say which regions the port took.
...
6. **The same recipe proves a region *free*.** Seed it, play through every path that could
   touch it, `read_memory` it back: every byte still `&A5` says nothing wrote there. Paradroid's
   stack-page measurement (`&0100-&017F` untouched in a 40-second session) is this. **List the
   paths that were not exercised** - Paradroid's doc says the lift, transfer and briefing paths
   were not, which is why its harness could never ship. A free-space claim without that list is
   incomplete.
