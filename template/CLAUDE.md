...
| Loader | One data file, `PANEL`, ZX02-compressed by `tools/make_disc.py`, staged at `LOADER_STAGE` = `&3000` (the blanked screen) and unpacked to `&4A00`. All disc access precedes `install_irq`, and `claim_nmi` follows the last of it |
| Other machines | Four defaults, each measured on jsbeeb 2026-09-11 (`../docs/hardware-facts.md`, "Target configurations"). Every catalogue address and the loader's OSFILE block are `&FFFFxxxx`, the host's, so a second processor does not take the files. OSBYTE 114,1 before the one `VDU 22`, so `*SHADOW` cannot put the display in shadow RAM. Every CRTC register is written after the mode change. `claim_nmi` (OSBYTE 143,12, then `RTI` at `&0D00`) replaces any `*TAPE`. Keep all four when changing the boot |
