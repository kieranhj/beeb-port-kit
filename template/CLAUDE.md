...
| `lib/` | forks of the kit's `beeb.h`, `irq`, `keydown`, `loader`, `zx02depack`, `boot_stamp` (the kit's copies are `.asm`/BeebASM; these are `.6502`/Baron, and the only differences are the `ASSERT` shim in `beeb.h.6502` and `BUILD_TIME` in `boot_stamp.6502`). Each header says what was measured and what the includer defines; keep the headers, note changes in them |
