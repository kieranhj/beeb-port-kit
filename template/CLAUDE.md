...
`!BOOT` is a `*RUN` stub (`--opt 2`, every build) that prints its own copy of the same bytes.
The stamp is written ONCE, in that macro, so the two cannot drift.

## Source organisation (`src/`)

| File | Contents |
|---|---|
| `main.6502` | constants, zero page, boot (blank, `VDU 22`, load the panel, fill the strip, `setup_display`, `install_irq`), the main loop, the `BUILD_STAMP` macro, and the five `SECTION`s that become files: `zeropage`, `Game`, `Panel`, `Info` (`INFO`, the stamp, assembled at `INFO_STAGE` = `&7E00`) and `Boot` (the `!BOOT` stub at `BOOT_STAGE` = `&0900`, where it can run) |
...
| `tools/verify_dynamic.mjs` | boots the disc in jsbeeb (found by `tools/jsbeeb_src.mjs`, which prints the version it loaded) and measures what the build *does* - the panel unpack, the field counter, the 25 Hz lock, the scroll under a held X - as one line of JSON. **The check to run when a change moves addresses**, which any edit does now that the zero page is allocated: `node tools/verify_dynamic.mjs build/game.ssd <scroll> <field_count> <frame_count>`, the addresses from `tools/listing.py symbols build/game.symbols.json` |
| `tools/listing.py` | reads Baron's symbol dump and its listing: `python tools/listing.py symbols build/game.symbols.json NAME` for any label, computed constant or `ZA_AUTO` address (the zero-page ones exist nowhere else), `python tools/listing.py stream build/game.lst` for the mechanical-change diff. **Take every address from here, never from a doc** |
