...
A port of *[the original]* to the BBC Micro, in 6502 assembly for BeebASM. **`PLAN.md` is the
...
| Loader | One data file, `PANEL`, ZX0-compressed by `tools/make_disc.py`, staged at `LOADER_STAGE` = `&3000` (the blanked screen) and unpacked to `&4A00`. All disc access precedes `install_irq` |
...
**`RELEASE` and `MASTER` are beebasm command-line symbols and every build passes both** -
beebasm has no `IFDEF` and refuses a symbol defined twice, so `main.asm` cannot carry a default.
A bare invocation, from the project root (INCLUDE and INCBIN resolve from the cwd):

```
C:\Users\khcon\OneDrive\BEEB\Bin\beebasm.exe -i src\main.asm -do build\GAME-RAW.SSD -opt 3 -D RELEASE=0 -D MASTER=0 -v
python tools\make_disc.py build\GAME-RAW.SSD build\GAME.SSD build\GAME-200K.SSD
```

**beebasm's own image is NOT bootable.** `make_disc.py` compresses `PANEL`, round-trips the
stream through `tools/zx0.py`, rewrites its catalogue load address to `LOADER_STAGE`, refuses a
stream that overlaps its output, lays the files out in boot order and pads to 200K. Hand
`build/GAME-200K.SSD` to an emulator. beebasm writes progress to stderr: in PowerShell do not
redirect that stream; check the exit code. Every compile flag that changes the disc is stamped
in `!BOOT` (`BOOT_FLAG` in `main.asm`); add any new `DEBUG_` flag to `DEBUG_ANY` and to the stamp.

## Source organisation (`src/`)

| File | Contents |
|---|---|
| `main.asm` | constants, zero page, boot (blank, `VDU 22`, load the panel, fill the strip, `setup_display`, `install_irq`), the main loop, the SAVEs, `!BOOT` at `BOOT_STAGE` = `&7E00` |
| `rupture.asm` | `rupt_vsync` and `rupt_timer`, the two-cycle rupture and its write-window rules; the two palettes (`PALWR`/`PALSET`, sixteen entries each) and the three T1 fires |
| `lib/` | forks of the kit's `beeb.h.asm`, `irq.asm`, `keydown.asm`, `loader.asm`, `zx0depack.asm`, `boot_stamp.asm`. Each header says what was measured and what the includer defines; keep the headers, note changes in them |
...
- Debug builds are switched by `DEBUG_` constants at the top of `main.asm`; every debug key
