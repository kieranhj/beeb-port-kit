...
| `tools/verify_dynamic.mjs` | boots the disc in jsbeeb and measures what the build *does* - the panel unpack, the field counter, the 25 Hz lock, the scroll under a held X - as one line of JSON. **The check to run when a change moves addresses**, which any edit does now that the zero page is allocated: `node tools/verify_dynamic.mjs build/GAME-200K.SSD <scroll> <field_count> <frame_count>`, the addresses from `tools/listing.py symbols` |
