...
beeb-port-kit: from tools/zx0.py, BYTE-IDENTICAL in both ports.
  https://github.com/kieranhj/paradroid-beeb/blob/main/tools/zx0.py
  https://github.com/kieranhj/edge-beeb/blob/master/tools/zx0.py
Proven: every stream on both shipping discs round-trips through decompress()
...
`zx0` with no flags). Change the depacker and you must change this.

compress() is O(n * max_offset) pure Python and takes seconds on a 16K bank;
the reference zx0.exe is what the ports call for the real streams, with this
module as the oracle. For a file already compressed by the exe, decompress()
is all you need.
"""
