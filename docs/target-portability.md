...
| 14 | **Master Compact** (ADFS, no DFS ROM) and the ADFS models | Can't read a DFS image at all. An ADFS version is real work, but **don't preclude it**: no `*DISC`, and nothing that assumes DFS's shapes. `OSBYTE 140` after the last load is fine - it is the filing system being asked to detach, not a filing system being forced - but an ADFS port that loads between levels has to move the call, not delete it | b-em preset 13; jsbeeb `B1770A`, `MasterADFS` | not tried | not tried |
| 15 | Booted from **MMFS, ADFS or Econet** rather than DFS | Anything that forces DFS (`*DISC`) breaks it. Kit default: `OSBYTE 140` after the last load, then claim the NMI - 140 selects the tape filing system rather than DFS, so a machine booted from MMFS or ADFS comes back to its own on the next BREAK | b2 (MMFS); jsbeeb `MasterANFS` | fixed (intro's `*TAPE`/`*DISC` removed) | not checked |
