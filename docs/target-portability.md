...
| 16 | **`*CONFIGURE LANG 11`** (Edit) or **a B with no BASIC ROM** | `*EXEC !BOOT` fails: it types into Edit, or stops at "Language?". Nobody really runs a machine like this (KC), but a `*RUN` boot (`*OPT 4,2`) needs no language and looks cleaner, so **the template's RELEASE build boots that way**: `!BOOT` is a stub that prints the stamp and `*RUN`s the game (`boot_stamp.6502`, `BOOT_RUN`). It's entered with **interrupts off** (P = `&35` at its first instruction, both models), so the MOS never ends the BREAK beep: it sounded until `install_irq` was made to silence the chip. OSBYTE 126 doesn't help there: no ESCAPE is pending at that point (`&FF` = 0), so it does nothing (`hardware-facts.md`). The DEV build keeps `*EXEC`, now four `*` commands - one of them `*TYPE INFO`, which prints the stamp from the disc file the RELEASE stub prints its own copy of (`INFO`, 2026-09-11) | jsbeeb: boot the release image, read the sound chip | deliberately left open | not checked |
