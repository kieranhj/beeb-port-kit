...
| **`release_fs` (OSBYTE 140) then `claim_nmi` (OSBYTE 143,12 and `RTI` at `&0D00`), after the LAST load** - `release_fs` **only if the game never touches the disc again**, since it selects the TAPE filing system and there is no way back; a port that loads between levels, streams off the disc or saves anything keeps the filing system and owns the machine instead, and then makes no OS call at all | The filing system keeps its hooks and its workspace. Any OS call the game makes afterwards - OSBYTE 229, `&0F`, 19 - may go through a filing system whose workspace the game has buried: Acorn's DFS hooks none of them, which is the only reason it appears to work, and Paradroid hung on hexwab's Opus 1770 with DDOS 3.45. See `hardware-facts.md` "The MOS and DFS" and `lib/loader.6502`'s header: a LOADER, which must keep the filing system, owns the machine and claims the NMI instead |
...
| 14 | **Master Compact** (ADFS, no DFS ROM) and the ADFS models | Can't read a DFS image at all. An ADFS version is real work, but **don't preclude it**: no `*DISC`, and nothing that assumes DFS's shapes. `OSBYTE 140` after the last load is fine - it is the filing system being asked to detach, not a filing system being forced - but only for a port that reads the disc once; one that loads between levels must not call it at all | b-em preset 13; jsbeeb `B1770A`, `MasterADFS` | not tried | not tried |
| 15 | Booted from **MMFS, ADFS or Econet** rather than DFS | Anything that forces DFS (`*DISC`) breaks it. Kit default, for a port that reads the disc only at boot: `OSBYTE 140` after the last load, then claim the NMI - 140 selects the tape filing system rather than DFS, so a machine booted from MMFS or ADFS comes back to its own on the next BREAK. A port that must keep loading calls neither and owns the machine instead | b2 (MMFS); jsbeeb `MasterANFS` | fixed (intro's `*TAPE`/`*DISC` removed) | not checked |
| 16 | **`*CONFIGURE LANG 11`** (Edit) or **a B with no BASIC ROM** | `*EXEC !BOOT` fails: it types into Edit, or stops at "Language?". Nobody really runs a machine like this (KC), but a `*RUN` boot (`*OPT 4,2`) needs no language and looks cleaner, so **every template build boots that way** (dev too, from 2026-09-13: `*EXEC` writes into `&1100-&1900` and cannot coexist with `OSBYTE 140` - `lib/loader.6502`): `!BOOT` is a stub that prints the stamp and `*RUN`s the game (`boot_stamp.6502`, `BOOT_RUN`). It's entered with **interrupts off** (P = `&35` at its first instruction, both models), so the MOS never ends the BREAK beep: it sounded until `install_irq` was made to silence the chip. OSBYTE 126 doesn't help there: no ESCAPE is pending at that point (`&FF` = 0), so it does nothing (`hardware-facts.md`). The stamp the boot prints is the same text `INFO` holds, and `*TYPE INFO` reads that back at any prompt | jsbeeb: boot the release image, read the sound chip | deliberately left open | not checked |
